...
@app.post("/write")
def write_memo(request: Request, title: str = Form(...), content: str = Form(...), db: Session = Depends(get_db)):
...
def read_memos(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(request, "memos.html", {"memos": memo_list})
